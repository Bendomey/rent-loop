...
func NewTenantAccountRouter(appCtx pkg.AppContext, handlers handlers.Handlers) func(r chi.Router) {
	return func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			...
			r.Get("/v1/public/bookings/{tracking_code}", handlers.BookingHandler.PublicGetBookingTracking)

			// Public tracking routes (no auth required)
			r.Get("/v1/tenant-applications/code/{code}", handlers.TenantApplicationHandler.GetTenantApplicationByCode)
			r.Patch(
				"/v1/tenant-applications/code/{code}",
				handlers.TenantApplicationHandler.UpdateTenantApplicationByCode,
			)
		...
		r.Group(func(r chi.Router) {
			...
			// tenant checklists
			r.Get("/v1/leases/{lease_id}/checklists", handlers.LeaseChecklistHandler.TenantListLeaseChecklists)
			r.Get(
				"/v1/leases/{lease_id}/checklists/{checklist_id}",
				handlers.LeaseChecklistHandler.TenantGetLeaseChecklist,
			)
			r.Post(
				"/v1/leases/{lease_id}/checklists/{checklist_id}/acknowledge",
				handlers.LeaseChecklistHandler.TenantAcknowledgeChecklist,
			)
			...
			r.Get("/v1/leases/{lease_id}/maintenance-requests/stats", handlers.MaintenanceRequestHandler.TenantStats)
			r.Get(
