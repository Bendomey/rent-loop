...
type OutputClientApplication struct {
	...
	SubType string `json:"sub_type" example:"ESTATE MANAGER"` // INDIVIDUAL = LANDLORD; COMPANY = PROPERTY_MANAGER | DEVELOPER | AGENCY
	...
	RegistrationNumber *string `json:"registration_number" example:"GR1234567890"`          // company registration number
	LogoURL            *string `json:"logo_url" example:"www.logo-url.com/logo.png"`        // company logo URL or individual profile picture URL
	Description        *string `json:"description" example:"Taking you to the next level!"` // company description or individual bio
	WebsiteURL         *string `json:"website_url" example:"www.company-url.com"`           // company website URL
	SupportEmail       *string `json:"support_email" example:"support@email.com"`           // company support email
	SupportPhone       *string `json:"support_phone" example:"+233 (0)12 345 6789"`         // company support phone number

	// individual specific fields
	DateOfBirth   *string `json:"date_of_birth" example:"2025-01-31"`                  // individual date of birth
	IDType        *string `json:"id_type" example:"GHANACARD"`                         // individual ID type (e.g., passport, driver's license)
	IDNumber      *string `json:"id_number" example:"GHA-123-456-7890"`                // individual ID number
	IDExpiry      *string `json:"id_expiry" example:"2040-12-31"`                      // individual ID expiry date
	IDDocumentURL *string `json:"id_document_url" example:"www.id-doc-url.com/id.pdf"` // URL to the scanned copy of the ID document
	...
	Status string `json:"status" example:"ClientApplication.Status.Approved"` // ClientApplication.Status.Pending | ClientApplication.Status.Approved | ClientApplication.Status.Rejected
