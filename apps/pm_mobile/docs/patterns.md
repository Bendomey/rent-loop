...
## Permission Gating
- **`PermissionGuard` / `hasPermission`** (`shared/permission_guard.dart`) — ports the web app's `PermissionGuard`/`useHasPermissions` pattern for hiding UI by client-level role (`ClientUserModel.role` — OWNER/ADMIN/STAFF). `PermissionGuard` wraps a widget and renders `SizedBox.shrink()` when unauthorized (Flutter can't return `null` from `build()`, unlike React); `hasPermission(ref, {roles})` is the raw boolean for building conditional lists. UI-hiding convenience only — never a substitute for backend authorization, which is enforced independently. Client-level only for now; a property-scoped variant (mirroring web's `PropertyPermissionGuard`, gated on `ClientUserProperty.role`) can follow the same shape once pm_mobile has a per-property role context to read from.
