...
export function PropertyManagersModule() {
	...
	const status =
		searchParams.get('status') ?? 'ClientApplication.Status.Approved'
		...
		() => [
			...
			},
			{
				...
				cell: ({ getValue }) => {
					...
					const config = STATUS_CONFIG[s] ?? { label: s, className: '' }
					return (
	...
	return (
		<main className="flex flex-col gap-6 px-4 py-8 md:px-8">
			<div>
				<TypographyH2>Property Managers</TypographyH2>
				<TypographyMuted>Manage all property manager accounts.</TypographyMuted>
			</div>

			<PropertyManagersController isLoading={isLoading} refetch={refetch} />
			...
			<div className="h-full w-full">
				<DataTable
					...
					isLoading={isLoading}
					refetch={refetch}
					error={error ? 'Failed to load property managers.' : undefined}
					dataResponse={{
						rows: data?.rows ?? [],
					...
					empty={{
						...
						description: query
							? `No property managers match "${query}".`
							: 'No property managers have been registered yet.',
