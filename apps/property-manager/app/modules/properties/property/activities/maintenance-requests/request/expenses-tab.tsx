...
import {
	type GenerateMaintenanceInvoicePayer,
...
function InvoiceFormSheet({
	...
	const handleGenerate = () => {
		const apiPayers: GenerateMaintenanceInvoicePayer[] = payers.map((p) => ({
...
export function ExpensesTab({ requestId, propertyId }: ExpensesTabProps) {
	...
	const [activeInvoiceExpenseId, setActiveInvoiceExpenseId] = useState<
		string | null
	>(null)
	const [pendingDelete, setPendingDelete] = useState<MaintenanceExpense | null>(
		null,
	)
		...
		createExpense.mutate(
			...
			},
			{
				onSuccess: (expense) => {
					...
					const apiPayers: GenerateMaintenanceInvoicePayer[] =
						values.payers.map((p) => ({
							payer_type: p.payer_type,
							payee_type: p.payee_type,
							amount: Math.round(parseFloat(p.amount) * 100),
	...
	const handleConfirmDelete = async () => {
		...
		try {
			...
			deleteExpense.mutate(
				{ id: requestId, property_id: propertyId, expense_id: expense.id },
				{
					onSuccess: () => {
