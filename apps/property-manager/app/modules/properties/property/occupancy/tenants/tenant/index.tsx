...
export function TenantModule() {
	...
	const baseUrl = `/properties/${clientUserProperty?.property?.id}/occupancy/tenants/${tenantId}`
	return (
				...
				<Tabs value={pathname}>
					<div className="overflow-x-auto pb-1">
						<TabsList>
							...
							{isBooking && (
								<Link to={`${baseUrl}/bookings`}>
									<TabsTrigger value={`${baseUrl}/bookings`}>
										Bookings
									</TabsTrigger>
								</Link>
							)}
							<Link to={`${baseUrl}/payments`}>
								<TabsTrigger value={`${baseUrl}/payments`}>
									Payments
								</TabsTrigger>
							</Link>
