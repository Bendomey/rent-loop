...
export function TenantApplicationOTPValidationModule({ referredBy, unitId }: { referredBy: string | null, unitId: string | null }) {
const navigate = useNavigate()
const [otp, setOtp] = useState('')
const [canResend, setCanResend] = useState(true)
const [resendCountdown, setResendCountdown] = useState(0)

const { goBack, goNext, formData, updateFormData, unblockNavigation } =
...
const verify = () => {
    ...
    if (otp !== '1234') {
      ...
      setTimeout(() => {
    navigate(`/tenants/apply/new?unit=${unitId}&referred_by=${referredBy}`)
      }, 0)
  ...
  return (
    <div className="mx-auto md:max-w-2xl flex w-full items-center justify-center">
  ...
  {error && (
    <TypographySmall className="mt-4 text-center text-destructive">
      {error}
    </TypographySmall>
  )}
