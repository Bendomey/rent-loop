...
import { z } from 'zod'
import { useUpdateClientUserRole } from '~/api/client-users'
import {
	AlertDialog,
	AlertDialogContent,
...
export default function EditMemberRoleModule({ data, opened, setOpened }: Props) {
	...
	const rhfMethods = useForm<FormSchema>({
		defaultValues: {
			role: data?.role === 'OWNER' ? 'ADMIN' : (data?.role ?? 'STAFF'),
		},
	...
	useEffect(() => {
		if (data) {
			reset({ role: data.role === 'OWNER' ? 'ADMIN' : (data.role ?? 'STAFF') })
		}
	}, [data, reset])

	const { mutate, isPending } = useUpdateClientUserRole()
