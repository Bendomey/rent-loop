...
import { useEffect, useMemo, useRef, useState, type AllHTMLAttributes, type ChangeEvent, type FocusEventHandler } from "react";
...
interface Props extends Omit<NativeInputProps, 'width' | 'height' | 'className'> {
...
export function ImageUpload({
    ...
    fileRatioError = fileName => `${fileName} must have a ratio of, ${validation?.imageRatio}. Please try again.`,
    fileCallback,
    hero = false,
    ...
    const [imageData, setImageData] = useState<ImageData>({ error, status: imageSrc ? 'resolved' : 'idle' });
    const [imageDimensions, setImageDimensions] = useState<ImageDimensions | null>(null);

    const handleButtonClick = () => (inputRef?.current ? inputRef.current.click() : undefined);
    ...
    useEffect(() => {
        if (imageSrc) {
            ...
            img.onerror = () => {
                if (isMounted) {
                    setImageData(prevState => ({ ...prevState, status: 'rejected', error: 'Failed to load image' }));
                }
            };
    ...
    const imageContent = useMemo(() => {
        const dismissImage = () => {
            ...
            setImageData(prevState => ({ ...prevState, file: undefined, status: 'idle' }));
        };
        switch (imageData.status) {
            ...
            case 'resolved':
                ...
                const shouldApplyBlur =
                    ...
                    (() => {
                        ...
                        // Consider it mismatched if the ratios are inverted or differ by more than 10%
                        const ratioThreshold = 0.2;
                        const ratioDifference = Math.abs(imageAspectRatio - aspectRatio) / aspectRatio;
                ...
                return (
                    <>
                        {shouldApplyBlur ? (
                            ...
                            <div className='relative w-full h-full overflow-hidden rounded-sm flex items-center justify-center'>
                                ...
                                <img
                                    ...
                                    onLoad={() => (imageData?.file?.imageSrc ? URL.revokeObjectURL(imageData.file.imageSrc) : undefined)}
                                    src={imgSrc || undefined}
                                    style={{ zIndex: 2 }}
                                />
                            </div>
                            ...
                            <img
                                ...
                                className={cn([
                                    ...
                                    (shape !== 'unset' && !aspectRatio) || !hero ? 'aspect-square' : '',
                        ...
                        {!disabled && !hideDismissIcon ? (
                            <button
                                ...
                                className='absolute -top-0.5 -right-0.5 rounded-[24px] !p-0 bg-white hover:!text-gray-500 z-10'
            ...
            default:
                return (
                    <ImageIcon className="size-8 text-gray-500" data-testid='placeholder image' />
                );
        }
    }, [
        imageData,
    ...
    const validateImageDimensions = ({ file, imageUrl }: ValidateImageDimensionsProps): Promise<boolean> => {
            ...
            img.onload = async () => {
                ...
                if (validation) {
                    if (validation?.maxByteSize && file.size > validation.maxByteSize) {
                        const error = fileMaxKbSizeError(file.name, Math.round(validation.maxByteSize / 1000));
                        ...
                        setImageData({ error: fileMinSizeError(file.name), file: undefined, status: 'rejected' });
                        return reject(false);
                    } else if (
                        (validation?.maxHeight && img.height > validation?.maxHeight) ||
                        (validation?.maxWidth && img.width > validation?.maxWidth)
                    ) {
                        setImageData({ error: fileMaxSizeError(file.name), file: undefined, status: 'rejected' });
                        return reject(false);
                    } else if (
                        validation?.imageRatio &&
                        ratio?.length === 2 &&
                        ratio[0] && ratio[1] && parseInt(ratio[0]) / parseInt(ratio[1]) !== img.width / img.height
                    ) {
                        setImageData({ error: fileRatioError(file.name), file: undefined, status: 'rejected' });
                        return reject(false);
                    }
                }
        ...
        try {
            ...
            if (files) {
                ...
                if (file) {
                    ...
                    if (isImageValidated) {
                        // TODO URL.createObjectURL is being depricated in some browsers see, https://developer.chrome.com/blog/chrome-71-deps-rems/#remove-urlcreateobjecturl-from-mediastream.
    ...
    const circleStyles = cn([shape === 'circle' && !hero ? 'rounded-full aspect-square' : '']);
    const squareStyles = cn([shape === 'square' && !hero ? 'aspect-square rounded-sm' : '']);
    const aspectRatioStyles = cn([aspectRatio && !hero ? 'rounded-[4px]' : '']);
    const unsetStyles = cn([
        shape === 'unset' && !hero ? 'rounded-1' : '',
        shape === 'unset' && imageData.status !== 'idle' && !hero ? 'aspect-auto' : '',
        shape === 'unset' && imageData.status !== 'resolved' && !hero ? 'aspect-square' : '',
    ]);
    const heroStyles = cn([hero ? 'rounded-sm aspect-video' : '']);
    ...
    return (
        <div className={cn([className])}>
            <div
                className={cn([
                    'relative border border-solid rounded-sm p-4',
                    inputContainerClassName,
                    errorMessage ? 'border-red-400' : 'border-gray-200',
                    disabled ? 'select-none bg-gray-50 pointer-events-none' : '',
                ])}
            >
                <Label htmlFor={name} className="mb-5">{label}</Label>

                <div className='flex relative gap-2 flex-wrap justify-center md:justify-start'>
                    <div
                        className={cn([
                            'flex justify-center items-center relative mbe-2 md:mbe-0 md:mie-2',
                            !hero && !aspectRatio ? 'md:w-1/2 w-4/5' : '',
                            hero && !aspectRatio ? 'w-full' : '',
                            disabled ? 'border-small border-solid border-tertiary-light' : '',
                        ...
                        style={aspectRatio ? { aspectRatio: String(aspectRatio), width: `${imageWidth}px` } : undefined}
                    >
                        {imageContent}
                    </div>
                    <Button
                        ...
                        variant='outline'
                    >
                        {imageData.status === 'resolved' ? changeImageButtonLabel : chooseImageButtonLabel}
                    </Button>
                    <input
            ...
            {errorMessage ? <TypographyMuted className='mt-1 text-red-600'>{errorMessage}</TypographyMuted> : null}
