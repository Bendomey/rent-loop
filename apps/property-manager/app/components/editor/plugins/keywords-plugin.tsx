...
export function KeywordsPlugin(): JSX.Element | null {
	...
	const getKeywordMatch = useCallback((text: string) => {
		...
		const secondMatch = matchArr[1]
		const thirdMatch = matchArr[2]

		if (secondMatch == null || thirdMatch == null) {
			return null
		}

		const hashtagLength = thirdMatch.length
		const startOffset = matchArr.index + secondMatch.length
