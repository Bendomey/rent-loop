...
test('the financial account carries from application to lease', async ({
	...
	await expect(page.getByText(/still owes you/i).first()).toBeVisible({
		timeout: 20_000,
	})
	...
	// Labels come from the plain wording the tab now uses. The figures they
	// name are unchanged.
	expect(amountFor(leaseText, 'still owes you')).toBe(chargedBefore)
	expect(amountFor(leaseText, 'paid you')).toBe(0)
